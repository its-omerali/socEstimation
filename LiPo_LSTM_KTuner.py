...
output_tuned_folder = os.path.join('basic_model_stats', 'tuned', 'KerasTuner', 'LiPo')
...
print(best_trial)
trials_csv_path = os.path.join(output_tuned_folder, 'LiPo_tuner_results.csv')
trials_df.to_csv(trials_csv_path, index=False)
print("Saved LiPo tuner results to:", trials_csv_path)
...
plt.title('LiPo - Hyperparameter Tuning Results', fontsize=14)
plt.xticks(rotation=45)
plt.tight_layout()
bar_plot_path = os.path.join(output_tuned_folder, 'LiPo_tuner_barplot.png')
plt.savefig(bar_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiPo tuner bar plot at:", bar_plot_path)
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training vs. Validation Loss - LiPo Best Model')
plt.legend()
plt.tight_layout()
best_loss_plot_path = os.path.join(output_tuned_folder, 'LiPo_best_model_loss.png')
plt.savefig(best_loss_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiPo best model loss plot at:", best_loss_plot_path)
...
print("LiPo Best Model Validation Metrics:")
...
metrics_csv_path = os.path.join(output_tuned_folder, 'LiPo_best_model_validation_metrics.csv')
metrics_df.to_csv(metrics_csv_path, index=False)
print("Saved LiPo best model validation metrics at:", metrics_csv_path)
