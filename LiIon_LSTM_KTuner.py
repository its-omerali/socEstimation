...
# Try to import KerasTuner (if using TF>=2.7, you might need "import keras_tuner as kt")
try:
    import keras_tuner as kt
except ImportError:
    import kerastuner as kt

# Define output folder for tuned model stats for LiIon and create it if it doesn't exist
output_tuned_folder = os.path.join('basic_model_stats', 'tuned', 'KerasTuner', 'LiIon')
os.makedirs(output_tuned_folder, exist_ok=True)

# Load normalized LiIon data from pickle file
with open('data/normalized/LiIon_normalized.pkl', 'rb') as f:
    df = pickle.load(f)

# Determine the current column (assume either "current" or "currents")
...
# Pad sequences to the maximum length (padding at the end)
...
# Split into training and validation sets at the experiment level (80/20 split)
...
# --- Define the model builder for KerasTuner ---
...
# Set up early stopping callback for tuner search
...
tuner = kt.RandomSearch(
    ...
    project_name='LiIon_tuning'
)

# Perform hyperparameter search
tuner.search(X_train, y_train, epochs=30, validation_data=(X_val, y_val), callbacks=[early_stop])
best_hps = tuner.get_best_hyperparameters(num_trials=1)[0]
print("Best hyperparameters for LiIon:", best_hps.values)

# --- Save all trial results into a CSV file ---
trial_results = []
for trial_id, trial in tuner.oracle.trials.items():
    hp_values = trial.hyperparameters.values
    score = trial.score  # lower score is better (MSE)
...
print(best_trial)
trials_csv_path = os.path.join(output_tuned_folder, 'LiIon_tuner_results.csv')
trials_df.to_csv(trials_csv_path, index=False)
print("Saved LiIon tuner results to:", trials_csv_path)

# --- Plot a bar chart comparing validation loss for each trial ---
...
plt.title('LiIon - Hyperparameter Tuning Results', fontsize=14)
plt.xticks(rotation=45)
plt.tight_layout()
bar_plot_path = os.path.join(output_tuned_folder, 'LiIon_tuner_barplot.png')
plt.savefig(bar_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiIon tuner bar plot at:", bar_plot_path)

# --- Build and train the best model further ---
...
plt.xlabel('Epoch')
plt.ylabel('Loss')
plt.title('Training vs. Validation Loss - LiIon Best Model')
plt.legend()
plt.tight_layout()
best_loss_plot_path = os.path.join(output_tuned_folder, 'LiIon_best_model_loss.png')
plt.savefig(best_loss_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiIon best model loss plot at:", best_loss_plot_path)
...
print("LiIon Best Model Validation Metrics:")
...
metrics_csv_path = os.path.join(output_tuned_folder, 'LiIon_best_model_validation_metrics.csv')
metrics_df.to_csv(metrics_csv_path, index=False)
print("Saved LiIon best model validation metrics at:", metrics_csv_path)
