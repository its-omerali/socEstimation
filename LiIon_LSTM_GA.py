...
# Define output folder for GA tuning results for LiIon and create it if it doesn't exist
output_ga_folder = os.path.join('basic_model_stats', 'tuned', 'GA', 'LiIon')
os.makedirs(output_ga_folder, exist_ok=True)

print("Loading normalized LiIon data...", flush=True)
with open('data/normalized/LiIon_normalized.pkl', 'rb') as f:
    df = pickle.load(f)

# Determine current column: prefer 'current' else 'currents'
...
print(f"LiIon GA Tuner: Number of experiments = {num_experiments}", flush=True)
print(f"LiIon GA Tuner: Maximum sequence length = {max_len}", flush=True)
...
# Register dropout in range [0, 0.99]
...
# Custom mutation: use mutGaussian then clip dropout to [0, 0.99]
...
print("Best individual (GA) for LiIon:", best_ind, "with fitness:", best_ind.fitness.values, flush=True)

# ---------------------------
# Save GA Tuning Results
# ---------------------------
ga_results_df = pd.DataFrame(ga_results)
ga_csv_path = os.path.join(output_ga_folder, 'LiIon_GA_results.csv')
ga_results_df.to_csv(ga_csv_path, index=False)
print("Saved LiIon GA tuning results to:", ga_csv_path, flush=True)
...
plt.title('LiIon GA Tuning Results', fontsize=14)
plt.tight_layout()
ga_barplot_path = os.path.join(output_ga_folder, 'LiIon_GA_barplot.png')
plt.savefig(ga_barplot_path, bbox_inches='tight')
plt.close()
print("Saved LiIon GA tuning bar plot at:", ga_barplot_path, flush=True)

best_units = int(best_ind[0])
best_dropout = best_ind[1]
print("Training final LiIon model with GA best parameters: units =", best_units, "dropout_rate =", best_dropout, flush=True)
...
plt.title('Training vs. Validation Loss - LiIon Final Model (GA)', fontsize=14)
plt.legend()
plt.tight_layout()
final_loss_plot_path = os.path.join(output_ga_folder, 'LiIon_final_model_loss.png')
plt.savefig(final_loss_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiIon final model loss plot at:", final_loss_plot_path, flush=True)
...
print("LiIon Final Model (GA) Validation Metrics:", flush=True)
...
final_metrics_csv_path = os.path.join(output_ga_folder, 'LiIon_final_model_validation_metrics.csv')
final_metrics_df.to_csv(final_metrics_csv_path, index=False)
print("Saved LiIon final model validation metrics at:", final_metrics_csv_path, flush=True)
