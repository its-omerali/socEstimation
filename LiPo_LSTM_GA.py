...
output_ga_folder = os.path.join('basic_model_stats', 'tuned', 'GA', 'LiPo')
os.makedirs(output_ga_folder, exist_ok=True)

print("Loading normalized LiPo data...", flush=True)
...
print(f"LiPo GA Tuner: Number of experiments = {num_experiments}", flush=True)
print(f"LiPo GA Tuner: Maximum sequence length = {max_len}", flush=True)
...
print("Best individual (GA) for LiPo:", best_ind, "with fitness:", best_ind.fitness.values, flush=True)

ga_results_df = pd.DataFrame(ga_results)
ga_csv_path = os.path.join(output_ga_folder, 'LiPo_GA_results.csv')
ga_results_df.to_csv(ga_csv_path, index=False)
print("Saved LiPo GA tuning results to:", ga_csv_path, flush=True)
...
plt.title('LiPo GA Tuning Results', fontsize=14)
plt.tight_layout()
ga_barplot_path = os.path.join(output_ga_folder, 'LiPo_GA_barplot.png')
plt.savefig(ga_barplot_path, bbox_inches='tight')
plt.close()
print("Saved LiPo GA tuning bar plot at:", ga_barplot_path, flush=True)

best_units = int(best_ind[0])
best_dropout = best_ind[1]
print("Training final LiPo model with GA best parameters: units =", best_units, "dropout_rate =", best_dropout, flush=True)
...
plt.title('Training vs. Validation Loss - LiPo Final Model (GA)', fontsize=14)
plt.legend()
plt.tight_layout()
final_loss_plot_path = os.path.join(output_ga_folder, 'LiPo_final_model_loss.png')
plt.savefig(final_loss_plot_path, bbox_inches='tight')
plt.close()
print("Saved LiPo final model loss plot at:", final_loss_plot_path, flush=True)
...
print("LiPo Final Model (GA) Validation Metrics:", flush=True)
...
final_metrics_csv_path = os.path.join(output_ga_folder, 'LiPo_final_model_validation_metrics.csv')
final_metrics_df.to_csv(final_metrics_csv_path, index=False)
print("Saved LiPo final model validation metrics at:", final_metrics_csv_path, flush=True)
